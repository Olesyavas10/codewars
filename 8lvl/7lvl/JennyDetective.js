...
function missingWord(nums, str) {
  let numsS=nums.sort((a,b)=>a-b) //по возрастанию числа в массиве
  let strL=str.toLowerCase()
  let arrayStr=strL.split(" ").join('')
  let output=[]
  for (let i=0; i<numsS.length; i++)
    {
      if (numsS[numsS.length-1]>(arrayStr.length-1))
        {
          return "No mission today"
        }
      else {
      output+=arrayStr[numsS[i]]   
      }
